...
export default function PollInteractiveClient({
  ...
  return (
    <div className="space-y-4">
      {errorMessage && (
        <div className="bg-red-50 border border-red-200 text-red-600 text-xs px-4 py-2.5 rounded-xl flex items-center gap-2">
            ...
            <button
              ...
              className={`w-full text-left relative overflow-hidden rounded-xl border p-4 transition-all duration-300 ${
                hasVoted
                  ...
                  : 'bg-white hover:bg-zinc-50 border-zinc-200 hover:border-zinc-300 text-zinc-700 hover:text-zinc-900 cursor-pointer active:scale-[0.99] shadow-sm'
              }`}
            >
              ...
              {hasVoted && (
                <div
                  className={`absolute top-0 left-0 bottom-0 transition-all duration-1000 ease-out z-0 ${
                    isUserVote ? 'bg-emerald-100' : 'bg-zinc-100'
                  }`}
              ...
              <div className="relative z-10 flex items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                  ...
                  {hasVoted ? (
                    ...
                    <div className="w-5 h-5 rounded-full border border-zinc-300 group-hover:border-zinc-400 transition-colors" />
                  )}
      ...
      {!isLoggedIn && (
        <div className="bg-zinc-50 border border-zinc-200 rounded-xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 mt-4">
          <div className="flex items-start gap-2.5">
            <Lock className="w-4 h-4 text-zinc-500 mt-0.5 shrink-0" />
            <div>
              <p className="text-xs font-bold text-zinc-800">Gunakan Hak Suara Warga</p>
              <p className="text-[11px] text-zinc-500">Anda perlu masuk akun terlebih dahulu untuk memberikan suara.</p>
            </div>
          </div>
          <Link
            href="/login?redirect=dashboard"
            className="inline-flex items-center justify-center gap-2 px-3 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white font-black text-xs transition-colors shrink-0 shadow-sm active:scale-95"
          >
